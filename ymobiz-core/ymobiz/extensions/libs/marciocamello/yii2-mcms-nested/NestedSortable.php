...
class NestedSortable extends \yii\base\Widget
{
    ...
    public function run()
    {
		...
		$view->registerJs("
			...
			var paramsSortable = '".json_encode($this->params)."';
		",$view::POS_END);
	...
	public function showNested($parentID)
	{
		...
		$model = $modelclass::find()->where([
			'parent' => $parentID
		])->orderBy('order asc,updated_at desc,created_at desc')->groupBy('unique_id');
	...
	public function renderNested()
	{
        ...
        if($this->join==false){
            $model = $modelclass::find()->where([
                'parent' => 0,
            ])->orderBy('order')->groupBy('unique_id');
        }else{
            $model = $modelclass::find()->where([
                'parent' => 0,
            ])->orderBy('order')->groupBy('unique_id')
                ->joinWith($this->join['relation'])->where($this->join['params']);
        }
