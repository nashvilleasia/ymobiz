...
(function ($) {
    ...
    $.fn.editableutils = {
        ...
        createInput: function(options) {
            ...
            //change wysihtml5 to textarea for jquery UI and plain versions
            if(type === 'file' && !$.fn.editabletypes[type]) {
                type = 'file';
            }
    ...
    $.extend(Checklist.prototype, {
        renderList: function() {
            ...
            /*<span class="custom-checkbox">
                ...
                <label for="other-shipping-address">click here if your shipping address is diferent from the one above</label>*/

            for(var i=0; i<this.sourceData.length; i++) {
                $label = $('<label for="'+this.sourceData[i].value+'">').append($('<input>', {
                        type: 'checkbox',
                        value: this.sourceData[i].value,
                        id: this.sourceData[i].value
                    }))
                    .append($('<span>').text(' '+this.sourceData[i].text));

                /*$label = '<span class="custom-checkbox">'+
                         '<input type="checkbox" value="'+this.sourceData[i].value+'" id="'+this.sourceData[i].value+'">'+
                         '</span>'+
                         '<label for="'+this.sourceData[i].value+'">'+this.sourceData[i].text+'</label>';*/
    ...
    Combodate.prototype = {
        ...
        getTemplate: function() {
            ...
            $.each(this.map, function(k, v) {
                ...
                tpl = tpl.replace('{'+token+'}', '<select class="'+k+' '+inputClass+'"></select>');
            });
...
(function ($) {
    ...
    $.fn.editableform.buttons = '<button type="submit" class="btn btn-primary editable-submit"><i class="glyphicon glyphicon-ok icon-white"></i></button>'+
        '<button type="button" class="btn editable-cancel"><i class="glyphicon glyphicon-remove icon-remove"></i></button>';
