...
class ymoPartnerMemberForm extends Model
{	
	public $id;
	public $user_id;
	public $name;
	...
	{
		$model = ymoUser::find()->where(['id'=>($id) ? $id : Yii::$app->user->id])->one();
    ...
    public function getPermissionsGroup()
    {
    	return array_diff(ArrayHelper::toArray(ArrayHelper::map(Yii::$app->authManager->getRoles(),'name', 'name')),[
    		'dev','admin','*','Guest Role','Partner Supervisor','Admin Superuser','Card Holder',
    		'Admin Supervisor','Staff Call','Staff Compliance','Staff Treasury','Staff Emission',
    		'Admin Ymobiz','Backoffice Ymobiz','Business Ymobiz','Marketing Ymobiz','Network Ymobiz','Settings Ymobiz'	
    	]);
    }
    ...
    public function newMember()
    {
    	...
    	if($userMember->validate()){
    		...
    		$clients->partner_id = Yii::$app->user->id;
    ...
    public function viewMembers()
    {
    	...
    	return $query->select('users.*')
	    	...
	    	->where(['users.group_id' => 'Seller', 'users.id'=>self::findMemberId()])
	    	->orWhere(['users.group_id' => 'Account' ,'users.id'=>self::findMemberId()])
		    ->andwhere('users.cluster_id = "'.Yii::$app->params['cluster_name'].'"')
	    	->andWhere('clients.partner_id = "'.Yii::$app->user->id.'"')
    ...
    {	
    	...
    	if($memberID!=false AND $match==false AND $search==false){
	    	...
	    	->from('ymodb_auth.ymo_user users')
	    	->where(['users.group_id' => 'Seller'])
	    	->orWhere(['users.group_id' => 'Account'])
		    ->andwhere('users.cluster_id = "'.Yii::$app->params['cluster_name'].'"')
	    	->andWhere('clients.partner_id = "'.Yii::$app->user->id.'"')
	    	->join('LEFT JOIN', 'ymodb_ymocard.ymo_orders orders', 'orders.client_id = users.id')
	    	->join('LEFT JOIN', 'ymodb_ymocard.ymo_clients clients', 'clients.user_id = users.id')
    ...
    public function searchMembers()
    {
    	if(Yii::$app->request->post('search')){
    		Yii::$app->controller->redirect(['default/members', 'search' => Yii::$app->request->post('search')]);
    	}
    	...
    	$provider = $query->select('users.*')
	    	...
	    	->orWhere(['like', 'users.status_code', str_replace('-', ' ', $match)])
	    	->orWhere(['like', 'clients.status_code', str_replace('-', ' ', $match)])
	    	->orWhere(['like', 'orders.status_compliance', str_replace('-', ' ', $match)])
	    	->join('LEFT JOIN', 'ymodb_ymocard.ymo_orders orders', 'orders.client_id = users.id')
	    	->join('LEFT JOIN', 'ymodb_ymocard.ymo_clients clients', 'clients.user_id = users.id')
	    	->groupBy('clients.user_id')
	    	->orderBy('users.created_at desc, users.updated_at desc')
