...
$cardID = (ymoCardsPayment::findCardId()) ? ymoCardsPayment::findCardId() : 'null';
...
$view->registerJs("
	...
	jQuery(document).on('change','.filter-card', function(e){
		...
		if(this.value!==''){
			...
			window.location = '/supervisor/treasury/filter/' + decodeURI(filterValue);
		}
	});	
...
<div class="col-md-6 ymo-column-left ymo-nopadding ymo-Panel">
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-list ymo-nopadding ymo-Panel">
        ...
        <div class="col-md-4 ymo-nopadding">
        	<a href="<?php echo \Yii::$app->urlManager->createUrl('/supervisor/treasury')?>" class="col-md-12 btn ymo-btn-light-pink"><?php echo Yii::$app->getModule('site')->ymoTranslate->t('site','app','refresh')?></a>
        </div>
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-table ymo-nopadding">
         ...
         if(Yii::$app->request->get('filter')==false){
         	$models = (Yii::$app->request->post('search') || Yii::$app->request->get('search')) ? $model->searchPaymentCards('default/treasury') : $model->listPaymentCards();
         }else{
         	$models = (Yii::$app->request->get('filter')) ? $model->filterPaymentCards('/supervisor/treasury') : false;
         }
            ...
            'columns' => [
                [
                	...
                	'value' => function ($model, $index, $widget) {
                		...
                		$status = ymoCardsPayment::checkTreasury($model->id);
                	
                        return '<span '.$status.'>' . ymoCardsPayment::hideCardNumber($model->cardnumber,'3','3') . " | " . 
                        Inflector::camel2words($model->title . ' ' . $model->name) . " | " .
                         $model->ordersOne->status_treasury . '</span>';
         	        }
                ],
                [
                    ...
                    'buttons' => ['view-card' => function ($url, $model) {
                            ...
                            return "<a $status href='".\yii\helpers\Url::toRoute(['default/treasury','cardid' => $model->id, 'search' => Yii::$app->request->get('search'), 'filter' => Yii::$app->request->get('filter'),  'page' => Yii::$app->request->get('page')])."' data-pjax=0>".Yii::$app->getModule('site')->ymoTranslate->t('site','app','check payment')."</a>";
                        }
                    ],
                    'template' => '{view-card}',
