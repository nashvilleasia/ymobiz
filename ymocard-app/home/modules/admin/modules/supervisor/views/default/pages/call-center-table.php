...
$modelCards = new ymoCardsSupervisor();
...
$view->registerJs("
	...
	jQuery(document).on('change','.filter-card', function(e){
		...
		if(this.value!==''){
			...
			window.location = '/supervisor/call-center/filter/' + decodeURI(filterValue);
		}
	});	
...
<div class="col-md-6 ymo-column-left ymo-nopadding ymo-Panel">
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-list ymo-nopadding ymo-Panel">
        ...
        <div class="col-md-4 ymo-nopadding">
        	<a href="<?php echo \Yii::$app->urlManager->createUrl('/supervisor/call-center')?>" class="col-md-12 btn ymo-btn-light-pink"><?php echo Yii::$app->getModule('site')->ymoTranslate->t('site','app','refresh')?></a>
        </div>
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-table ymo-nopadding">
         ...
         if(Yii::$app->request->get('filter')==false){
         	$models = (Yii::$app->request->post('search') || Yii::$app->request->get('search')) ? $modelCards->searchCards('default/call-center') : $modelCards->listCards();
         }else{
         	$models = (Yii::$app->request->get('filter')) ? $modelCards->filterCards('/supervisor/call-center') : false;
         }
            ...
            'columns' => [
                [
                	...
                	'value' => function ($model, $index, $widget) {
                		...
                		$status = ymoCardsSupervisor::checkStatusCard($model->id);
                ...
                [
                    ...
                    'buttons' => ['view-card' => function ($url, $model) {
                            ...
                            return "<a $status href='".\yii\helpers\Url::toRoute(['default/call-center','cardid' => $model->id, 'search' => Yii::$app->request->get('search'), 'filter' => Yii::$app->request->get('filter'), 'page' => Yii::$app->request->get('page')])."' data-pjax=0>".Yii::$app->getModule('site')->ymoTranslate->t('site','app','view details')."</a>";
                        }
                    ],
                    'template' => '{view-card}',
    ...
    <div class="col-md-11 col-md-offset-1 ymo-search ymo-nopadding ymo-Panel">
        <div class="col-md-5 ymo-nopadding ymo-Panel">
            <?php
                echo \yii\helpers\Html::dropDownList('status',(Yii::$app->request->get('filter')) ? Yii::$app->request->get('filter') : null ,$modelCards->listStatus(),[
