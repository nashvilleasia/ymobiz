...
$modelCompliance = new ymoCardsCompliance();

$cardID = (ymoCardsCompliance::findCardId()) ? ymoCardsCompliance::findCardId() : 'null';
...
$view->registerJs("
	...
	jQuery(document).on('change','.filter-card', function(e){
		...
		if(this.value!==''){
			...
			window.location = '/supervisor/compliance/filter/' + decodeURI(filterValue);
		}
	});	
...
<div class="col-md-6 ymo-column-left ymo-nopadding ymo-Panel">
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-list ymo-nopadding ymo-Panel">
        ...
        <div class="col-md-4 ymo-nopadding">
        	<a href="<?php echo \Yii::$app->urlManager->createUrl('/supervisor/compliance')?>" class="col-md-12 btn ymo-btn-light-pink"><?php echo Yii::$app->getModule('site')->ymoTranslate->t('site','app','refresh')?></a>
        </div>
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-table ymo-nopadding">
         ...
         if(Yii::$app->request->get('filter')==false){
         	$models = (Yii::$app->request->post('search') || Yii::$app->request->get('search')) ? $modelCompliance->searchComplianceCards('default/compliance') : $modelCompliance->listComplianceCards();
         }else{
         	$models = (Yii::$app->request->get('filter')) ? $modelCompliance->filterComplianceCards('/supervisor/compliance') : false;
         }
            ...
            'columns' => [
                [
                	...
                	'value' => function ($model, $index, $widget) {
                		...
                		$status = ymoCardsCompliance::checkCompliance($model->id);
                	
                        return '<span '.$status.'>' . ymoCardsCompliance::hideCardNumber($model->cardnumber,'3','3') . " | " . 
                        Inflector::camel2words($model->title . ' ' . $model->name) . " | " .
                         $model->ordersOne->status_compliance . '</span>';         	        
                	}
                ],
                [
                    ...
                    'buttons' => ['view-card' => function ($url, $model) {
                            ...
                            return "<a $status href='".\yii\helpers\Url::toRoute(['default/compliance','cardid' => $model->id, 'search' => Yii::$app->request->get('search'), 'filter' => Yii::$app->request->get('filter'), 'page' => Yii::$app->request->get('page')])."' data-pjax=0>".Yii::$app->getModule('site')->ymoTranslate->t('site','app','check compliance')."</a>";
                        }
                    ],
                    'template' => '{view-card}',
    ...
    <div class="col-md-11 col-md-offset-1 ymo-search ymo-nopadding ymo-Panel">
        <div class="col-md-5 ymo-nopadding ymo-Panel">
            <?php
                echo \yii\helpers\Html::dropDownList('status',(Yii::$app->request->get('filter')) ? Yii::$app->request->get('filter') : null ,$modelCompliance->listStatus(),[
