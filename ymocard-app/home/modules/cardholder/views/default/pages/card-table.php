...
$cardID = (ymoCards::findCardId()) ? ymoCards::findCardId() : 'null';
...
$view->registerJs("
	...
	jQuery(document).on('change','.filter-card', function(e){
		...
		if(this.value!==''){
			...
			window.location = '/card-holder/filter/' + decodeURI(filterValue);
		}
	});	
...
<div class="col-md-6 ymo-column-left ymo-nopadding ymo-Panel">
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-list ymo-nopadding ymo-Panel">
        ...
        <h2 class="col-md-8 ymo-title-a">
            <?php echo Yii::$app->getModule('site')->ymoTranslate->t('site','app','your cards')?>
        </h2>
        
        <div class="col-md-4 ymo-nopadding">
       	 	<a href="<?php echo \Yii::$app->urlManager->createUrl('card-holder/order')?>" class="col-md-12 btn ymo-btn-light-pink" style="margin-bottom: 3px;"><?php echo Yii::$app->getModule('site')->ymoTranslate->t('site','app','order new ymocard')?></a>
        	<a href="<?php echo \Yii::$app->urlManager->createUrl('card-holder')?>" class="col-md-12 btn ymo-btn-light-pink"><?php echo Yii::$app->getModule('site')->ymoTranslate->t('site','app','refresh')?></a>
        </div>
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-table ymo-nopadding">
         ...
         if(Yii::$app->request->get('filter')==false){
         	$models = (Yii::$app->request->post('search') || Yii::$app->request->get('search')) ? $model->searchCardsByClient() : $model->listCardsByClient();
         }else{
         	$models = (Yii::$app->request->get('filter')) ? $model->filterCardsByClient() : false;
         }
