...
$view->registerJs("
	...
	jQuery(document).on('change','.filter-card', function(e){
		...
		if(this.value!==''){
			...
			window.location = '/staff-emission/filter/' + decodeURI(filterValue);
		}
	});
...
<div class="col-md-6 ymo-column-left ymo-nopadding ymo-Panel">
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-list ymo-nopadding ymo-Panel">
        ...
        <div class="col-md-4 ymo-nopadding">
        	...
        	<a href="<?php echo \Yii::$app->urlManager->createUrl('/staff-emission/order-emission')?>" class="col-md-12 btn ymo-btn-light-pink" style="margin-bottom: 5px;"><?php echo Yii::$app->getModule('site')->ymoTranslate->t('site','app','emit all ymocards')?></a>
        	<?php } ?>
        	<a href="<?php echo \Yii::$app->urlManager->createUrl('/staff-emission')?>" class="col-md-12 btn ymo-btn-light-pink"><?php echo Yii::$app->getModule('site')->ymoTranslate->t('site','app','refresh')?></a>
        </div>
    ...
    <div class="col-md-11 col-md-offset-1 ymo-card-table ymo-nopadding">
         ...
         if(Yii::$app->request->get('filter')==false){
         	$models = (Yii::$app->request->post('search') || Yii::$app->request->get('search')) ? $model->searchEmissionCards('default/index') : $model->listEmissionCards();
         }else{
         	$models = (Yii::$app->request->get('filter')) ? $model->filterEmissionCards('/staff-emission') : false;
         }
            ...
            'columns' => [
                ...
                [
                    ...
                    'buttons' => ['view-card' => function ($url, $model) {
                            ...
                            return "<a $status href='".\yii\helpers\Url::toRoute(['default/index','cardid' => $model->id, 'search' => Yii::$app->request->get('search'), 'filter' => Yii::$app->request->get('filter'), 'page' => Yii::$app->request->get('page')])."' data-pjax=0>".Yii::$app->getModule('site')->ymoTranslate->t('site','app',($status) ? 'order emission' : 'order emission sent')."</a>";
